...
# 5) Installe Gulp 3 et le CLI globalement pour garantir que la tâche "gulp" soit disponible
RUN npm install -g gulp@3.9.1 gulp-cli

# 6) Copie le reste des fichiers source
COPY src ./src

# 7) Expose le port BrowserSync (défini à 8000 dans gulpfile.js)
EXPOSE 3000

# 8) Lance Gulp directement (utilise le CLI global)
